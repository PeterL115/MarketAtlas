...
# ============================================================
# normalize helpers for dict ordering and correctness
# ============================================================

def _norm_zone(zlo: Any, zhi: Any) -> tuple[Optional[float], Optional[float]]:
    lo = _sf(zlo)
    hi = _sf(zhi)
...
def _zone_fields(lv: Optional[Dict[str, Any]], dist_atr: Optional[float]) -> Dict[str, Any]:
    if not lv:
        return {
            "id": None,
            "kind": None,
    ...
    lo, hi = _norm_zone(lv.get("zone_low"), lv.get("zone_high"))
    return {
        "id": _level_name(lv),
        "kind": lv.get("kind"),
        "zone_high": hi,
...
def _band_fields(lo: Any, hi: Any) -> Dict[str, Optional[float]]:
    low, high = _norm_zone(lo, hi)
...
_RTH_BARS_PER_DAY = 78  # 6.5 h × 12 bars/h at 5-min resolution


def _generate_intraday_guidance(
    likelihoods: Dict[str, Any],
    diagnostics: Dict[str, Any],
    range_context: Dict[str, Any],
) -> Dict[str, Any]:
    """
    Derive actionable options structure suggestion from intraday signals.

    Inputs used:
      - likelihoods: stall / breakout_up / breakout_down scores
      - diagnostics: iv, rv_5m, er, sigma_rem_atr, bars_remaining, pos_rth_range
      - range_context: remaining_session_45/68/95 bands (for strike placement)
    """
    stall = _sf(likelihoods.get("stall")) or 0.0
    bu    = _sf(likelihoods.get("breakout_up")) or 0.0
    bd    = _sf(likelihoods.get("breakout_down")) or 0.0

    iv            = _sf(diagnostics.get("iv"))
    rv_5m         = _sf(diagnostics.get("rv_5m"))
    er            = _sf(diagnostics.get("er"))
    sigma_rem_atr = _sf(diagnostics.get("sigma_rem_atr"))
    bars_rem      = diagnostics.get("bars_remaining")
    pos_rth_range = _sf(diagnostics.get("pos_rth_range"))

    # ── IV vs intraday RV premium assessment ──────────────────────────────────
    # rv_5m is per-bar std; annualise: rv_ann = rv_5m × sqrt(252 × 78)
    iv_rv_ratio: Optional[float] = None
    premium_env = "neutral"
    if iv is not None and rv_5m is not None and rv_5m > 0:
        rv_ann = rv_5m * math.sqrt(252 * _RTH_BARS_PER_DAY)
        iv_rv_ratio = round(iv / rv_ann, 3)
        if iv_rv_ratio > 1.15:
            premium_env = "rich"    # IV > realised: sell premium
        elif iv_rv_ratio < 0.85:
            premium_env = "cheap"   # IV < realised: buy premium

    # ── Late-session gamma warning ────────────────────────────────────────────
    gamma_warning = isinstance(bars_rem, int) and 0 < bars_rem <= 6

    # ── Directional bias from movement likelihoods ────────────────────────────
    if bu > 0.60 and bu > bd + 0.10:
        direction = "bullish"
    elif bd > 0.60 and bd > bu + 0.10:
        direction = "bearish"
    else:
        direction = "neutral"

    # ── Avoid flag ────────────────────────────────────────────────────────────
    avoid = gamma_warning or (sigma_rem_atr is not None and sigma_rem_atr < 0.10)

    # ── Recommended structure ─────────────────────────────────────────────────
    er_str = f"ER={er:.2f}" if er is not None else ""
    ivr_str = f"IV/RV={iv_rv_ratio:.2f}" if iv_rv_ratio is not None else ""

    if avoid:
        if gamma_warning:
            structure = "avoid"
            structure_reason = (
                f"Extreme gamma risk — only {bars_rem} bars left. No new positions."
            )
        else:
            structure = "avoid"
            structure_reason = (
                "Insufficient remaining move (σ < 0.10 ATR). Risk/reward does not support entry."
            )
    elif sigma_rem_atr is not None and sigma_rem_atr < 0.20:
        structure = "iron_condor" if premium_env in ("rich", "neutral") else "wait"
        structure_reason = (
            "Very low remaining move expected — tight iron condor if IV supports, otherwise stand aside."
        )
    elif stall > 0.60 and premium_env == "rich":
        structure = "iron_condor"
        structure_reason = (
            f"High stall ({stall:.0%}) + elevated IV ({ivr_str}) "
            "→ sell iron condor at ±1σ range edges."
        )
    elif stall > 0.60:
        structure = "iron_condor"
        structure_reason = (
            f"High stall probability ({stall:.0%}) → iron condor or credit spread inside range bands."
        )
    elif direction == "bullish" and premium_env == "rich":
        structure = "bull_put_spread"
        structure_reason = (
            f"Bullish bias ({bu:.0%} breakout-up) + rich premium ({ivr_str}) "
            "→ sell bull put spread below nearest support."
        )
    elif direction == "bearish" and premium_env == "rich":
        structure = "bear_call_spread"
        structure_reason = (
            f"Bearish bias ({bd:.0%} breakout-down) + elevated IV ({ivr_str}) "
            "→ sell bear call spread above nearest resistance."
        )
    elif direction == "bullish":
        structure = "long_call_spread"
        structure_reason = (
            f"Bullish breakout bias ({bu:.0%}), premium not elevated → debit call spread."
        )
    elif direction == "bearish":
        structure = "long_put_spread"
        structure_reason = (
            f"Bearish breakout bias ({bd:.0%}) → debit put spread for defined-risk directional play."
        )
    elif premium_env == "rich":
        structure = "short_strangle"
        structure_reason = (
            f"No directional edge ({er_str}) but IV elevated ({ivr_str}) "
            "→ short strangle at ±1σ edges."
        )
    else:
        structure = "wait"
        structure_reason = (
            "No clear edge — direction unclear and IV not elevated. "
            "Wait for a level test or directional confirmation."
        )

    # ── Strike placement notes from range bands ───────────────────────────────
    strike_notes: List[str] = []
    b45 = range_context.get("remaining_session_45") or {}
    b68 = range_context.get("remaining_session_68") or {}
    b95 = range_context.get("remaining_session_95") or {}
    ...
    if lo45 is not None and hi45 is not None:
        strike_notes.append(
            f"Short strikes (IC body): inside {lo45:.0f}–{hi45:.0f} (45% remaining range)"
        )
    ...
    if lo95 is not None and hi95 is not None:
        strike_notes.append(
            f"Max loss boundary: {lo95:.0f}–{hi95:.0f} (95% / 2σ)"
        )
    if pos_rth_range is not None:
        if pos_rth_range > 0.67:
            pos_desc = "upper third of day's range — consider skewing short strikes down"
        elif pos_rth_range < 0.33:
            pos_desc = "lower third of day's range — consider skewing short strikes up"
        else:
            pos_desc = "mid-range — strikes can be placed symmetrically"
        strike_notes.append(f"Price in {pos_desc} (pos={pos_rth_range:.2f}).")
...
def generate_intraday_insight_one(
    ...
    if iv is None:
        iv = _sf(feat.get("iv"))
    if iv is None:
        iv = _sf((feat.get("features") or {}).get("iv"))
    ...
    if rem_lo is not None and rem_hi is not None:
        lo68, hi68 = _norm_zone(rem_lo, rem_hi)
        if lo68 is not None and hi68 is not None:
            pos_rem_68 = _band_position(last, lo68, hi68)
    ...
    if sigma_pts is not None and sigma_pts > 0:
        for central in p.extra_range_bands:
            ...
            extra_bands[k] = _band_fields(bd["low"], bd["high"])
            lo_v, hi_v = _norm_zone(bd["low"], bd["high"])
            extra_positions[k] = None if (lo_v is None or hi_v is None) else _band_position(last, lo_v, hi_v)
    ...
    for k in final_order:
        if k == "68":
            if rem_lo is not None and rem_hi is not None:
                range_context["remaining_session_68"] = _band_fields(rem_lo, rem_hi)
            elif "68" in extra_bands:
                range_context["remaining_session_68"] = extra_bands["68"]
        elif k == "95":
            if rem95_lo is not None and rem95_hi is not None:
                range_context["remaining_session_95"] = _band_fields(rem95_lo, rem95_hi)
    ...
    for k in final_order:
        ...
        if k == "68" and pos_rem_68 is not None:
            ...
            if pos is None:
                pos = _band_position(last, lo2, hi2)
    ...
    base_out: Dict[str, Any] = {
        ...
        "levels_context": {
            "nearest_support": _zone_fields(sup, sup_dist_atr),
            "nearest_resistance": _zone_fields(res, res_dist_atr),
        },
    ...
    base_out["guidance"] = _generate_intraday_guidance(
        likelihoods=base_out["likelihoods"],
        diagnostics=base_out["diagnostics"],
        range_context=range_context,
    )

    base_out["pivot_levels"] = read_pivot_levels_for_day(cfg, ticker=ticker, asof_day=day)
