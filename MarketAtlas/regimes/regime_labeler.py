...
def write_regime_outputs(cfg: AppConfig, ticker: str, asof_day: str, df_labels: pd.DataFrame) -> Dict[str, str]:
    ...
    asof_day_str = str(r.get("day"))
    regime_str   = str(r.get("regime"))

    payload: Dict[str, Any] = {
        "ticker": ticker,
        "day": asof_day_str,
        # These two keys are what eod_run / tomorrow_range expect:
        "asof_regime":    regime_str,
        "asof_label_day": asof_day_str,
        # Keep legacy key for any code that still reads "regime" directly
        "regime": regime_str,
